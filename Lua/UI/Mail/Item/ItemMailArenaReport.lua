--author: 	Amu
--time:		2020-06-30 20:21:14

local GD = _G.GD

local ItemMailArenaReport = fgui.extension_class(GComponent)
fgui.register_extension("ui://Mail/itemMailArenaReport", ItemMailArenaReport)

function ItemMailArenaReport:ctor()
    ...
    self._textExplainH = self._textExplain.height
...
function ItemMailArenaReport:InitEvent()
...
function ItemMailArenaReport:SetData(index, _info, player, showType)
    self.info = _info
    self.player = player
...
function ItemMailArenaReport:InitBaseData(list, infoList)
...
function ItemMailArenaReport:initListView(  )
    ...
    if report.Attacker.UserId == self.player then
        self._textExplain.text = StringUtil.GetI18n("configI18nCommons", "UI_ARENA_BATTLE_TIPS26", 
                        {num1 = math.ceil(report.Attacker.RealInjured), 
                        num2 = math.ceil(report.Attacker.BeastRealInjured)})
    else
        self._textExplain.text = StringUtil.GetI18n("configI18nCommons", "UI_ARENA_BATTLE_TIPS27")
    end

    local _textH = 0
    if self._textExplainH < self._textExplain.displayObject.height then
        _textH = self._textExplain.displayObject.height - self._textExplainH
    end
    ...
    self._massLine.y = self._massLineY + _massNum * self.massH + _textH
...
function ItemMailArenaReport:IsAttack()
