--[[
    author:{maxiaolong}
    time:2020-06-02 10:55:05
    function:{阵亡纪念日}
]]
local MemorialDay = fgui.extension_class(GComponent)
fgui.register_extension("ui://Welfare/MemorialDay", MemorialDay)
local WelfareModel = import("Model/WelfareModel")
local cutData = nil
local personData, unionData = nil
--默认为第一页
local cutIndex = 0
function MemorialDay:ctor()
    self._textSalvation = self:GetChild("textSalvation")
    self._textSalvation.text = StringUtil.GetI18n(I18nType.Commmon, "ACTIVITY_INVERSTIGATION2_NAME")
    self._textDes = self:GetChild("textDescribe")
    self._textDes.text = StringUtil.GetI18n(I18nType.Commmon, "ACTIVITY_INVERSTIGATION2_DESC")
    ...
    self._listView.itemRenderer = function(index, item)
        item:SetData(self.itemData[index + 1], WelfareModel.TemplateType.MemorialDay)
    end

    self:AddEvent(
        EventDefines.MemorialDayRefresh,
        function()
    ...
    --设置Banner
    self._banner.icon = UITool.GetIcon(GlobalBanner.FlagDay)
...
function MemorialDay:TagChage(index)
    self._c1.selectedIndex = index
    self:RefreshList(index)
end

function MemorialDay:OnOpen()
    self:RefreshShow(true, 0)
end

function MemorialDay:RefreshShow(playAnim, c1Page)
    self:SetShow(true)
...
function MemorialDay:SetShow(isShow)
    self.visible = isShow
...
function MemorialDay:RefreshList(index)
    cutIndex = index
